...
def customersupdate():
    ...
    xd=e4.get()
    xf=e5.get()
    sql = "update customers set custName = '%s', address = '%s', phoneNo = '%s', city = '%s' where custid = %d"%(xb,xc,xd,xf,xcb)
    cur.execute(sql)
    db.commit()
    db.close()
...
Bt=Button(t,text = "Find",fg="green",command = customersfind)
Bt.place(x=100,y=400)
Bt2=Button(t,text = "Update",fg="blue",command = customersupdate)
Bt2.place(x=200,y=400)
Bt3=Button(t,text = "New",fg="orange",command = nw)
Bt3.place(x=300,y=400)
Bt4=Button(t,text = "Close",fg="red",command = btclose)
